...
class Constrained_Planner(object):
    ...
    def __init__(self, obs_map, init_pos, goal, constraints, out_paths=None,
        ...
        # assert(len(con_get_robots(constraints)) == 1)
        # Store as tuple, as just need as dictionary key
        self.rob_id = con_get_robots(constraints)
        self.conn_8 = conn_8
        ...
        if self.constraints is not None:
            self.max_t = con_get_max_time(self.constraints) + 1
            if self.max_t == 1:
        ...
        prev = goal_node
        for t in range(self.con_max_t + 1, self.max_t + 1):
    ...
    def expand(self, node, open_list):
        ...
        for i in coords:
            ...
            if (new_cost < new_node.cost):
                ...
                if con_is_constrained(self.constraints, node.coord[-1],
                                          i[:2], node.coord[:2]):
    ...
    def get_neighbors(self, coord):
        ...
        coords = self.sub_search[self.policy_key].get_forwards_neighbors(
            coord, self.max_t)
        return [c for c in coords if not con_is_constrained(
    ...
    def get_limited_offset_neighbors(self, coord, max_offset, min_offset=0):
        ...
        out = []
        for offset, neighbors in node.offset_neighbors.items():
...
class Constrained_Forwards_Planner(object):
    ...
    def __init__(self, obs_map, init_pos, goal, constraints, sub_search=None,
        ...
        self.goal_t = con_get_max_time(self.constraints) + 1
    ...
    def expand(self, node, open_list):
        ...
        for i in coords:
            ...
            if (temp_cost < new_node.cost):
                ...
                if con_is_constrained(self.constraints, nt, node.coord, i):
                    # Hit constraint, can skip rest of tests
                    continue
